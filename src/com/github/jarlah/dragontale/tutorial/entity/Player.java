    ...
    public Player(TileMap tm) {
        ...
        try {
            ...
            for (int i = 0; i < animations.length; i++) {
                AnimationInfo an = animations[i];
                BufferedImage[] bi = new BufferedImage[an.numFrames];
                for (int j = 0; j < an.numFrames; j++) {
                    bi[j] = spritesheet.getSubimage(j * an.width, i * an.height, an.width, an.height);
                }
                sprites.add(bi);
            }
